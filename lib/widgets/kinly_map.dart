...
/// tramite controller.toScreenLocationBatch — una query diretta al motore
/// nativo ("dove sta questa coordinata sullo schermo ADESSO"), non una
/// proiezione reimplementata in Dart: qualunque rotazione/inclinazione
/// della camera resta corretta gratis, perché a calcolarla è la mappa
/// stessa. Due motivi per non usare più Symbol nativi:
/// 1) vivono nella superficie grafica GL che Android/iOS può distruggere e
///    ricreare senza preavviso quando l'app va in background per liberare
///    memoria — un widget Flutter no.
/// 2) trackCameraPosition sotto è quello che rende questo aggiornamento
///    fluido invece che "in ritardo": senza, il plugin MapLibre scarta
///    del tutto l'evento onCameraMove durante un trascinamento (lo nota
///    solo a gesto finito, tramite onCameraIdle), e i pin sembrerebbero
///    seguire il dito per poi scattare al posto giusto al rilascio.
/// La mappa nativa resta comunque il riferimento per le tile di sfondo,
/// il riempimento delle aree sicure e i cerchi (posizione approssimativa,
/// anteprima ricerca): niente di tutto ciò che deve restare sempre
/// visibile in modo affidabile.
    ...
    return Stack(
      children: [
        MapLibreMap(
          ...
          // Senza trackCameraPosition: true, il plugin scarta del tutto
          // l'evento onCameraMove durante un trascinamento (lo nota solo a
          // gesto finito, tramite onCameraIdle): i pin si sarebbero
          // riposizionati solo al rilascio del dito, sembrando "seguirlo"
          // nel frattempo invece di restare ancorati al posto giusto.
